...
@st.cache_data
def load_bg():
    return get_base64_of_bin_file("assets/Imdb_Image.png")
...
def get_connection():
    # Absolute path to the SSL certificate, assuming 'assets' is in the project root
    # Streamlit Cloud/Local execution usually sets the CWD to the project root
    ssl_ca = "assets/isrgrootx1.pem"
    
    # Fallback if file doesn't exist (though it should)
    ssl_config = {"ssl": {"ca": ssl_ca}} if os.path.exists(ssl_ca) else {"ssl": True}
...
is_filter_mode = st.session_state.main_selection == "Filter Movies"
is_static_page = st.session_state.active_page in [
    "about", "products", "services", "contact", "privacy", "terms"
]
...
if st.session_state.main_selection == "Search Options":
...
# -------------------- PLOTS --------------------
if st.session_state.main_selection == "Plot with Details":
...
# -------------------- FILTER MOVIES --------------------
if st.session_state.main_selection == "Filter Movies":
    ...
    genres_df = run_query("SELECT DISTINCT Genre FROM imdb_movies_list")
    genre_options = ["All"] + genres_df["Genre"].tolist()
    ...
    try:
        ...
        if not df.empty:
            ...
            st.markdown("""<div style="
                        ...
                        """,
                        unsafe_allow_html=True)  # Display a message if no results are found
...
if st.session_state.active_page == "home" and "Filter Options":
    ...
    if st.session_state.active_page == "Filter Options":
        ...
        if main_selection == "Search Options":
            ...
            min_rating = st.sidebar.slider(
                "Minimum Rating", 0.0, 10.0, 5.0, step=0.1, key="min_rating"
            )
            max_rating = st.sidebar.slider(
                "Maximum Rating", 0.0, 10.0, 10.0, step=0.1, key="max_rating"
            )

            min_duration = st.sidebar.slider(
                "Minimum Duration (mins)", 0, 300, 90, key="min_duration"
            )
            max_duration = st.sidebar.slider(
                "Maximum Duration (mins)", 0, 300, 180, key="max_duration"
            )

            min_votes = st.sidebar.number_input(
                "Minimum Votes", min_value=0, value=1000, key="min_votes"
            )
            max_votes = st.sidebar.number_input(
                "Maximum Votes", min_value=0, value=1000000, key="max_votes"
            )
            ...
            if main_selection == "Filter Movies":
                # Prepare the SQL query for filtering
                query = """
                    SELECT Title, Ratings, Votings, Duration, Genre
                    FROM imdb_movies_list
        ...
        if not df.empty:
            ...
            # ✅ Single styled table with formatting
            styled_df = (
                df.style
                .format({"Ratings": "{:.2f}"})
                .set_properties(**{
    ...
    # -------------------- PLOTS --------------------
        ...
        # -------------------- FILTER MOVIES --------------------
        #if st.session_state.active_page == "home" and main_selection == "Filter Movies":
